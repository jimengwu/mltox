...
def getArguments():
    ...
    parser.add_argument("-ni", "--niter", dest="niter", default=500000, type=int)
    parser.add_argument(
...
for repeat in range(args.repeat):
    ...
    if args.t_ls == "median":
        ...
        lst = []
        for file in os.listdir("T_models/T_null"):
            if file.endswith(".txt"):
                lst.append(
                    pd.read_csv(os.path.join("T_models/T_null", file))
                    .sort_values(
        ...
        threshold_ls = max(lst, key=lst.count)

    print(threshold_ls)
    hyper_params_tune = {
        # T_median_ml, T_tuned_ml, T_num_ml:
...
# python RASAR_mulneigh_bi_cte_T_models.py -i /local/wujimeng/code_jimeng/data/invivo/invivo_repeated_w_invitro_eawag.csv -r 20 -n 1  -ni 25 -ah best  -o "V_models/repeat"
